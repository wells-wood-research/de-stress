...
//		if (strcmp(an, "CB") != 0) continue;
		int skip = 0;
		if (Etype == 1){
			if(strcmp(an, "CA") != 0) skip = 1;
		} else if(Etype == 2){
			if(strcmp(an, "CB") != 0) skip = 1;
			if(strcmp(rn,"GLY")==0 && strcmp(an, "CA")==0) skip = 0;
		}
		if(skip) continue;
//
...
int main(int argc, char *argv[]){
	if(argc < 2) {fprintf(stderr, "usage: %s dfire_pair.lib [-CA|-CB] PDBs\n", argv[0]); exit(1);}
	rdlib(argv[1]);		// read the potential file into matrix
	for(int i=2; i<argc; i++){
		if(string(argv[i]) == "-CA") {Etype = 1; continue;}
		else if(string(argv[i]) == "-CB") {Etype = 2; continue;}
		calENE(argv[i]);		// cal the energy
	}
}
